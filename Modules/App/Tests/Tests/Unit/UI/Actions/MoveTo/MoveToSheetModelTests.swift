...
class MoveToSheetStateStoreTests: BaseTestCase {
    ...
    func testState_WhenMailboxTypeIsMessageAndViewAppear_ItReturnsMoveToActions() {
        ...
        let sut = sut(input: .init(sheetType: .moveTo, ids: ids, type: .message(isStandaloneMessage: false)))
    ...
    func testState_WhenCreateFolderActionIsHandled_ItPresentsCreateFolderLabelModal() {
        let sut = sut(input: .init(sheetType: .moveTo, ids: [], type: .message(isStandaloneMessage: false)))
    ...
    func testAction_WhenInboxIsTapped_ItMovesMessageToInbox() {
        let sut = sut(input: .init(sheetType: .moveTo, ids: [.init(value: 1)], type: .message(isStandaloneMessage: false)))
    ...
    func testAction_WhenItsStandaloneMessageInConveration_WhenInboxIsTapped_ItMovesMessageToInbox() {
        let sut = sut(input: .init(sheetType: .moveTo, ids: [.init(value: 1)], type: .message(isStandaloneMessage: true)))
