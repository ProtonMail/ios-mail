...
extension ComposerContactProvider {
    ...
    static var mockInstance: Self {
        ...
        // Generate random single contacts
        let names = [
            ...
            "Aaron Taylor Green", "Aaron Taylor Parker", "Abigail", "Abigail Anderson",
            ...
            "Abigail Johnson", "Abigail Johnson Green", "Abigail Martinez", "Abigail Rodriguez",
            "Abigail Taylor Collins", "Abigail Taylor Green", "Alex", "Alexander", "Alexander Anderson",
        ...
        let emailPrefix = [
            "aaron.brown", "aaron.clark", "aaron.harris", "aaron.hernandez",
             "aaron.miller", "aaron.rodriguez", "abigail.allen", "abigail.hall",
        ...
        for (index, _) in (1...2000).enumerated() {
            let email = "\(emailPrefix.randomElement()!)_\(index)\(emailDomain.randomElement()!)"
            let name = Bool.random() ? "\(names.randomElement()!)" : email
            let contactType = ComposerContactType.single(ComposerContactSingle(name: name, email: email))
            contacts.append(ComposerContact(type: contactType))
        }
        ...
        for group in groups {
            contacts.append(ComposerContact(type: group))
        }

        // Sort contacts alphabetically by name or email
        contacts.sort {
...
extension ComposerContactType {
    // Helper to extract name or email for sorting
