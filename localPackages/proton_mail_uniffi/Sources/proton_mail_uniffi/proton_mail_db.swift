...
fileprivate extension RustBuffer {
    ...
    static func from(_ ptr: UnsafeBufferPointer<UInt8>) -> RustBuffer {
        try! rustCall { ffi_proton_mail_db_rustbuffer_from_bytes(ForeignBytes(bufferPointer: ptr), $0) }
    }

    // Frees the buffer in place.
    // The buffer must not be used after this is called.
    func deallocate() {
        try! rustCall { ffi_proton_mail_db_rustbuffer_free(self, $0) }
    }
}
...
private var initializationResult: InitializationResult {
    ...
    let scaffolding_contract_version = ffi_proton_mail_db_uniffi_contract_version()
