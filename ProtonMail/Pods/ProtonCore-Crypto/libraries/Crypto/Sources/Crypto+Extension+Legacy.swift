...
extension Crypto {
    ...
    public func decrypt(encrypted message: String, privateKey: String, passphrase: String) throws -> String {
        ...
        let newKey = try throwing { error in CryptoGo.CryptoNewKeyFromArmored(privateKey, &error) }
        ...
        let privateKeyRing = try throwing { error in CryptoGo.CryptoNewKeyRing(unlockedKey, &error) }
        let pgpMsg = try throwing { error in CryptoGo.CryptoNewPGPMessageFromArmored(message, &error) }

        guard let plainMessageString = try privateKeyRing?.decrypt(pgpMsg, verifyKey: nil, verifyTime: CryptoGo.CryptoGetUnixTime()).getString() else {
    ...
    func decrypt(encrypted message: String, privateKeys binKeys: [Data], passphrase: String) throws -> String {
        for binKey in binKeys {
            do {
                let newKey = try throwing { error in CryptoGo.CryptoNewKey(binKey.mutable as Data, &error) }
    ...
    func decrypt(encrypted binMessage: Data, privateKey: String, passphrase: String) throws -> String {
        ...
        let pgpMsg = CryptoGo.CryptoNewPGPMessage(binMessage.mutable as Data)
    ...
    public func decryptNonOptional(encrypted message: String,
        ...
        for binKey in binKeys {
            do {
                ...
                let verifierKey = try throwing { error in CryptoGo.CryptoNewKey(verifierBinKey.mutable as Data, &error) }

                let verifierKeyRing = try throwing { error in CryptoGo.CryptoNewKeyRing(verifierKey, &error) }
    ...
    public func decryptNonOptional(encrypted message: String,
                                   ...
                                   privateKey armorKey: String,
                                   passphrase: String, verifyTime: Int64) throws -> CryptoPlainMessage {
        let newKey = try throwing { error in CryptoGo.CryptoNewKeyFromArmored(armorKey, &error) }
    ...
    public func decryptVerifyNonOptional(encrypted message: String,
        ...
        let verified = try throwing { error in CryptoGo.HelperDecryptExplicitVerify(pgpMsg, privateKeyRing, verifierKeyRing, verifyTime, &error) }
    ...
    public func decryptVerifyNonOptional(encrypted message: String,
        ...
        let verifierKey = try throwing { error in CryptoGo.CryptoNewKeyFromArmored(publicKey, &error) }
    ...
    private func encrypt(input: Either<String, Data>, privateKey signerPrivateKey: String, passphrase: String) throws -> String {
        let privateKey = try throwing { error in CryptoGo.CryptoNewKeyFromArmored(signerPrivateKey, &error) }

        let passSlice = passphrase.data(using: .utf8)
        let unlockedKey = try privateKey?.unlock(passSlice)

        let publicKeyData = try unlockedKey?.getPublicKey()
        let publicKey = try throwing { error in CryptoGo.CryptoNewKey(publicKeyData, &error) }
        let publicKeyRing = try throwing { error in CryptoGo.CryptoNewKeyRing(publicKey, &error) }

        var plainMessage: CryptoPlainMessage?
        switch input {
        ...
        let signerKeyRing = try throwing { error in CryptoGo.CryptoNewKeyRing(unlockedKey, &error) }
    ...
    private func encrypt(input: Either<String, Data>, publicKey: String, privateKey signerPrivateKey: String = "", passphrase: String = "") throws -> String {
        let newKey = try throwing { error in CryptoGo.CryptoNewKeyFromArmored(publicKey, &error) }

        guard let key = newKey else {
            throw CryptoError.couldNotCreateKey
        }

        let publicKeyRing = try throwing { error in CryptoGo.CryptoNewKeyRing(key, &error) }
        ...
        if !signerPrivateKey.isEmpty {
            let signerKey = try throwing { error in CryptoGo.CryptoNewKeyFromArmored(signerPrivateKey, &error) }
    ...
    public func decryptNonOptional(encrypted: String, token: String) throws -> String {
        ...
        let pgpMsg = try throwing { error in CryptoGo.CryptoNewPGPMessageFromArmored(encrypted, &error) }
    ...
    public func decryptAttachmentNonOptional(keyPacket: Data, dataPacket: Data, privateKey: String, passphrase: String) throws -> Data {
        ...
        let passSlice = passphrase.data(using: .utf8)
        let unlockedKey = try key?.unlock(passSlice)
        let keyRing = try throwing { error in  CryptoGo.CryptoNewKeyRing(unlockedKey, &error) }

        let splitMessage = CryptoGo.CryptoNewPGPSplitMessage(keyPacket.mutable as Data, dataPacket.mutable as Data)
    ...
    public func decryptAttachmentNonOptional(keyPacket: Data, dataPacket: Data, privateKey binKeys: [Data], passphrase: String) throws -> Data {
        for binKey in binKeys {
            do {
                let key = try throwing { error in CryptoGo.CryptoNewKey(binKey.mutable as Data, &error) }
    ...
    public func decryptAttachmentNonOptional(encrypted: String, privateKey: String, passphrase: String) throws -> Data {
        ...
        let splitMessage = try throwing { error in CryptoGo.CryptoNewPGPSplitMessageFromArmored(encrypted, &error) }
    ...
    public static func signDetachedNonOptional(plainData: Data, privateKey: String, passphrase: String) throws -> String {
        ...
        let plainMessage = CryptoGo.CryptoNewPlainMessage(plainData.mutable as Data)
    ...
    public func signDetached(plainText: String, privateKey: String, passphrase: String) throws -> String {
        ...
        let unlockedKey = try key?.unlock(passSlice)
        let keyRing = try throwing { error in CryptoGo.CryptoNewKeyRing(unlockedKey, &error) }
        let plainMessage = CryptoGo.CryptoNewPlainMessageFromString(plainText.trimTrailingSpaces())
        let pgpSignature = try keyRing!.signDetached(plainMessage)
        let signature = try throwing { error in pgpSignature.getArmored(&error) }
    ...
    private func verifyDetached(signature: String, input: Either<String, Data>, publicKey: String, verifyTime: Int64) throws -> Bool {
        ...
        switch input {
        case .left(let plainText): plainMessage = CryptoGo.CryptoNewPlainMessageFromString(plainText.trimTrailingSpaces())
        case .right(let plainData): plainMessage = CryptoGo.CryptoNewPlainMessage(plainData.mutable as Data)
        }

        let signature = try throwing { error in CryptoGo.CryptoNewPGPSignatureFromArmored(signature, &error) }
    ...
    public func buildKeyRingNonOptional(adding keys: [Data]) throws -> CryptoKeyRing {
        ...
        for key in keys {
            do {
                let keyToAdd = try throwing { error in CryptoGo.CryptoNewKey(key, &error) }
    ...
    public func buildPrivateKeyRingNonOptional(adding keys: [Data], passphrase: String) throws -> CryptoKeyRing {
        ...
        for key in keys {
            do {
                let newKey = try throwing { error in CryptoGo.CryptoNewKey(key, &error) }
    ...
    public func buildPrivateKeyRing(keys: [(privateKey: String, passphrase: String)]) throws -> CryptoKeyRing {
        ...
        for key in keys {
            ...
            do {
                let lockedKey = try throwing { error in CryptoGo.CryptoNewKeyFromArmored(key.privateKey, &error) }
