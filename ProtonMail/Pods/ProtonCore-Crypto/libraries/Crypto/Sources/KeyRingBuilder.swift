...
public final class KeyRingBuilder {
    ...
    public func buildPublicKeyRing(armoredKeys: [ArmoredKey]) throws -> CryptoKeyRing {
        let keyRing = try throwingNotNil { error in CryptoGo.CryptoNewKeyRing(nil, &error) }
        var keyParsingErrors = [Error]()
        for armoredKey in armoredKeys {
            do {
                let keyToAdd = try throwingNotNil { error in CryptoGo.CryptoNewKeyFromArmored(armoredKey.value, &error) }
    ...
    public func buildPrivateKeyRingUnlock(privateKeys: [DecryptionKey]) throws -> CryptoKeyRing {
        ...
        for key in privateKeys {
            ...
            do {
                let lockedKey = try throwing { error in CryptoGo.CryptoNewKeyFromArmored(key.privateKey.value, &error) }
