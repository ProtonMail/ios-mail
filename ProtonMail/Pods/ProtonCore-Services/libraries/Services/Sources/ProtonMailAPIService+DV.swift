...
extension PMAPIService {
    ...
    func deviceVerificationHandler<T>(method: HTTPMethod,
                                      path: String,
                                      parameters: Any?,
                                      headers: [String: Any]?,
        ...
        if self.isDeviceVerifyProcessing.transform({ $0 }) {
            ...
            self.deviceVerificationProcess(method: method,
                                           path: path,
                                           parameters: parameters,
                                           headers: headers,
                                           authenticated: authenticated,
    ...
    private func deviceVerificationProcess<T>(method: HTTPMethod,
        ...
        // return completion if humanDelegate in not present
        guard let parameters = dvResponse.parameters else {
        ...
        dvSynchronizingQueue.async {
            ...
            self.dvCompletionQueue.async {
                let solvedChallenge = self.humanDelegate?.onDeviceVerify(parameters: parameters)
