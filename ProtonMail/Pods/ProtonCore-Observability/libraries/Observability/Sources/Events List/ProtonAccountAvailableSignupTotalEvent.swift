...
extension ObservabilityEvent where Payload == PayloadWithLabels<ProtonAccountAvailableSignupLabels> {
    public static func protonAccountAvailableSignupTotal(status: SuccessOrFailureStatus) -> Self {
        .init(name: "ios_core_proton_account_available_signup_total", labels: .init(status: status))
    }
}
