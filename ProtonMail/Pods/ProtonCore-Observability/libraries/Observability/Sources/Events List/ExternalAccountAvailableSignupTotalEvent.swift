...
extension ObservabilityEvent where Payload == PayloadWithLabels<ExternalAccountAvailableLabels> {
    public static func externalAccountAvailableSignupTotal(status: SuccessOrFailureStatus) -> Self {
        .init(name: "ios_core_external_account_available_signup_total", labels: .init(status: status))
    }
}
