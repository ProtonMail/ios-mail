...
public protocol AccountManagerUIProtocl: class {
    func reload()
    func dismiss()
}

final public class AccountManagerVC: UIViewController {
...
extension AccountManagerVC: UITableViewDataSource, UITableViewDelegate, AccountmanagerUserCellDelegate {
    ...
    func removeAccount(of userID: String) {
        guard let _ = self.viewModel.getAccountData(of: userID) else {
            return
        }
        self.checkRemoveWill(userID: userID)
    }
