...
final class MenuViewModel: NSObject {
    ...
    private var contextLabelUpdateFetcher: NSFetchedResultsController<NSFetchRequestResult>?
...
extension MenuViewModel: NSFetchedResultsControllerDelegate {
    func controllerDidChangeContent(_ controller: NSFetchedResultsController<NSFetchRequestResult>) {
        if controller == self.labelUpdateFetcher || controller == self.contextLabelUpdateFetcher {
            _ = self.updateUnread().done {
                self.delegate?.updateMenu(section: nil)
            }
            return
...
extension MenuViewModel {
    ...
    private func observeContextLabelUnreadUpdate() {
        ...
        let strComp = NSSortDescriptor(key: ContextLabelUpdate.Attributes.labelID,
                                       ...
                                       selector: #selector(NSString.localizedCaseInsensitiveCompare(_:)))
        fetchRequest.sortDescriptors = [strComp]
        self.contextLabelUpdateFetcher = NSFetchedResultsController(fetchRequest: fetchRequest, managedObjectContext: moc, sectionNameKeyPath: nil, cacheName: nil)
        self.contextLabelUpdateFetcher?.delegate = self
        
        guard let fetcher = self.contextLabelUpdateFetcher else {return}
        do {
            try fetcher.performFetch()
        } catch let ex as NSError {
            PMLog.D("MenuVM fetch label unread from coredata error: \(ex)")
