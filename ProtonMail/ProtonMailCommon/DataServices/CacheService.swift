    ...
    func move(message: Message, from fLabel: String, to tLabel: String) -> Bool {
        ...
        context.performAndWait {
            ...
            if let lid = msgToUpdate.add(labelID: tLabel) {
                ...
                var labelsFound = msgToUpdate.getNormalLableIDs()
            ...
            do {
                if let messages = try GRTJSONSerialization.objects(withEntityName: Message.Attributes.entityName, fromJSONArray: messagesArray, in: self.context) as? [Message] {
                    ...
                    if let lastMsg = messages.last, let firstMsg = messages.first {
                        self.updateLastUpdatedTime(labelID: labelID, isUnread: isUnread, startTime: firstMsg.time ?? Date(), endTime: lastMsg.time ?? Date(), msgCount: messagesCount, msgType: .message)
                    }
...
extension CacheService {
    ...
    func updateCounterSync(markUnRead: Bool, on message: Message, context: NSManagedObjectContext) {
        ...
        for lID in labelIDs {
            let unreadCount: Int = lastUpdatedStore.unreadCount(by: lID, userID: self.userID, type: .message)
            var count = unreadCount + offset
            if count < 0 {
                count = 0
            }
            lastUpdatedStore.updateUnreadCount(by: lID, userID: self.userID, count: count, type: .message, shouldSave: false)
    ...
    func updateCounterSync(plus: Bool, with labelID: String, context: NSManagedObjectContext) {
        let offset = plus ? 1 : -1
        // Message Count
        let unreadCount: Int = lastUpdatedStore.unreadCount(by: labelID, userID: self.userID, type: .message)
        var count = unreadCount + offset
        if count < 0 {
            count = 0
        }
        lastUpdatedStore.updateUnreadCount(by: labelID, userID: self.userID, count: count, type: .message, shouldSave: true)
