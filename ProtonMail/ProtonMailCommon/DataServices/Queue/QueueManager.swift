...
final class QueueManager: Service {
    ...
    init(messageQueue: PMPersistentQueueProtocol, miscQueue: PMPersistentQueueProtocol) {
    ...
    @objc(_TtCC5Share12QueueManager4Task) final class Task: NSObject, NSCoding {
        ...
        required convenience init?(coder: NSCoder) {
            guard let uuid = coder.decodeObject(forKey: "uuid") as? UUID,
                  ...
                  let isConversation = coder.decodeObject(forKey: "isConversation") as? Bool
            else { return nil }
