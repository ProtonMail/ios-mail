...
class LabelsDataService: Service, HasLocalStorage {
    ...
    func unreadCount(by lableID: String, userID: String? = nil) -> Promise<Int> {
        ...
        switch viewMode {
        case .conversation:
            return lastUpdatedStore.unreadCount(by: lableID, userID: userID ?? self.userID, type: .conversation)
        case .singleMessage:
            return lastUpdatedStore.unreadCount(by: lableID, userID: userID ?? self.userID, type: .message)
        }
    }
