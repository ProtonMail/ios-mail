...
public final class Conversation: NSManagedObject {
    ...
    enum Attributes {
        ...
        static let contextLabel = "contextLabels"
...
//    @NSManaged public var addressID: String?
    @NSManaged public var contextNumAttachments: NSNumber?
    @NSManaged public var contextNumMessages: NSNumber?
    @NSManaged public var contextNumUnread: NSNumber?
    @NSManaged public var contextSize: NSNumber?
    @NSManaged public var contextTime: Date?
    ...
    @NSManaged public var contextLabels: NSSet
...
//    func update(from tempConversation: Conversation, withContextLabels newContextLabels: Array<ContextLabel>) throws {
//        guard self.conversationID == tempConversation.conversationID else {
//            throw Errors.attemptToMergeUnmatchingConversations
//        }
//
//        self.numUnread = tempConversation.numUnread
//        self.numMessages = tempConversation.numMessages
//        self.numAttachments = tempConversation.numAttachments
//        self.order = tempConversation.order
//        self.recipients = tempConversation.recipients
//        self.senders = tempConversation.senders
//        self.size = tempConversation.size
//
//        self.contextLabels = NSSet(array: newContextLabels)
//        newContextLabels.forEach {
//            $0.conversations = self
//        }
//    }
}
...
extension Conversation {
    func getNumAttachments(labelID: String) -> Int {
        guard let contextLabels = self.contextLabels.allObjects as? [ContextLabel] else {
            return 0
        }
    ...
    func getLabels() -> [Label] {
        ...
        let labelIDs = self.contextLabels.compactMap{ ($0 as? ContextLabel)?.labelID }
...
extension Conversation {
    ...
    func applySingleMarkAsChanges(unRead: Bool, labelID: String) {
        let numUnread = self.numUnread.intValue
        let labels = self.mutableSetValue(forKey: "contextLabels")
        let contextNumUnread = labels.compactMap{$0 as? ContextLabel}.filter{ $0.labelID == labelID }.first?.unreadCount ?? 0
        
        let updatedNumUnread = unRead ? numUnread + 1 : max(numUnread - 1, 0)
        self.numUnread = NSNumber(value: updatedNumUnread)
    ...
    func applyMarksAsChanges(unRead: Bool, labelID: String) {
        ...
        let updatedNumUnread = unRead ? numUnread + 1 : 0
        self.numUnread = NSNumber(value: updatedNumUnread)
    ...
    func applyLabelChangesOnOneMessage(labelID: String, apply: Bool) {
        let labels = self.mutableSetValue(forKey: "contextLabels").compactMap{ $0 as? ContextLabel }
    ...
    func applyLabelChanges(labelID: String, apply: Bool) {
        if apply {
            if self.contains(of: labelID) {
                if let target = self.contextLabels.compactMap({ $0 as? ContextLabel }).filter({ $0.labelID == labelID }).first {
            ...
            if self.contains(of: labelID), let label = contextLabels.compactMap({ $0 as? ContextLabel }).filter({ $0.labelID == labelID }).first {
                self.mutableSetValue(forKey: "contextLabels").remove(label)
            }
        }
    }
