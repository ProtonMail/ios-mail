...
extension Message {
    ...
    // Compute the UUID generated by the server for a given message
    // it allows us to clean up notifications when marking as read without relying on remote notifications
    var notificationId: String? {
        guard let hexStr = Data(base64Encoded: messageID, options: .ignoreUnknownCharacters)?.stringFromToken(),
              hexStr.count > 19 else {
            return nil
        }

        let firstPart = hexStr[hexStr.startIndex...hexStr.index(hexStr.startIndex, offsetBy: 7)]
        let secondPart = hexStr[hexStr.index(hexStr.startIndex, offsetBy: 8)...hexStr.index(hexStr.startIndex, offsetBy: 11)]
        let thirdPart = hexStr[hexStr.index(hexStr.startIndex, offsetBy: 12)...hexStr.index(hexStr.startIndex, offsetBy: 15)]
        let fourthPart = hexStr[hexStr.index(hexStr.startIndex, offsetBy: 16)...hexStr.index(hexStr.startIndex, offsetBy: 19)]
        let uuid = "\(firstPart)-\(secondPart)-\(thirdPart)-\(fourthPart)-\("000000000000")"
