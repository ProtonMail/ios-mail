...
extension MessageEntity {
    ...
    func getSentLockType(email: String) -> PGPType {
        ...
        if let senderInfo = self.sender,
           senderInfo.email == email {
            if self.flag.contains(.autoReplied) {
        ...
        let authKey = MessageHeaderKey.pmRecipientAuthentication
        let encryptionKey = MessageHeaderKey.pmRecipientEncryption
        guard let authentication = self.getHeaderValue(headerKey: authKey,
                                                       subKey: email),
              let encryption = self.getHeaderValue(headerKey: encryptionKey,
                                                   subKey: email) else {
                  return .none
              }
        ...
        if authentication == "pgp-pm" {
            return encryption == "pgp-pm-pinned" ? .internal_trusted_key: .internal_normal
        }
        ...
        if authentication == "pgp-eo" { return .eo }

        if authentication == "none" {
            if encryption == "pgp-pm" {
                return .internal_normal
            }
            if encryption == "pgp-mime" || encryption == "pgp-inline" {
                return .pgp_encrypted
            }
            if encryption == "pgp-mime-pinned" || encryption == "pgp-inline-pinned" {
                return .pgp_encrypt_trusted_key
            }
    ...
    private func getHeaderValue(headerKey: String, subKey: String) -> String? {
        guard let rawValue = self.parsedHeaders[headerKey] as? String,
              let normalBody = rawValue.removingPercentEncoding else { return nil }
