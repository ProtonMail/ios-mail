                            ...
                            if let lastConversation = conversations.last, let firstConversation = conversations.first {
                                ...
                                if isUnread {
                                    ...
                                    if updateTime.isUnreadNew {
                                        updateTime.unreadStart = firstConversation.time ?? Date()
                                    }
                                    if let time = lastConversation.time, (updateTime.unreadEndTime.compare(time) == .orderedDescending) || updateTime.unreadEndTime == .distantPast {
                                        updateTime.unreadEnd = time
                                    }
                                } else {
                                    //Update normal query time
                                    if updateTime.isNew {
                                        updateTime.start = firstConversation.time ?? Date()
