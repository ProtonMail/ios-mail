...
class MessageDataService : Service, HasLocalStorage {
    ...
    func fetchMessages(byLable labelID : String, time: Int, forceClean: Bool, isUnread: Bool, completion: CompletionBlock?) {
            ...
            self.apiService.exec(route: getLatestEventID) { (task, IDRes: EventLatestIDResponse) in
                if !IDRes.eventID.isEmpty {
                    ...
                    self.cleanMessage().then { (_) -> Promise<Void> in
                        self.lastUpdatedStore.removeUpdateTime(by: self.userID, type: .message)
                        self.lastUpdatedStore.removeUpdateTime(by: self.userID, type: .conversation)
                        return self.contactDataService.cleanUp()
                    }.ensure {
                        self.fetchMessages(byLable: labelID, time: time, forceClean: false, isUnread: false, completion: completionWrapper)
                ...
                if eventsRes.refresh.contains(.all) || eventsRes.refresh.contains(.mail) || (eventsRes.code == 18001) {
                    ...
                    self.apiService.exec(route: getLatestEventID) { (task, eventIDResponse: EventLatestIDResponse) in
                        ...
                        self.cleanMessage().then {
                            ...
                            self.fetchMessages(byLable: labelID, time: 0, forceClean: false, isUnread: false, completion: completionWrapper)
                            self.contactDataService.fetchContacts(completion: nil)
                            self.labelDataService.fetchV4Labels().cauterize()
                        }.cauterize()
                    }
     ...
     */
    func fetchedResults(by labelID: String, viewMode: UserInfo.ViewMode, isUnread: Bool = false) -> NSFetchedResultsController<NSFetchRequestResult>? {
        switch viewMode {
        case .singleMessage:
        ...
        case .conversation:
            ...
            if isUnread {
                fetchRequest.predicate = NSPredicate(format: "(ANY contextLabels.labelID = %@) AND (%K == %@) AND (%K > 0)",
                                                     labelID, Conversation.Attributes.userID, self.userID, Conversation.Attributes.numUnread)
            } else {
                fetchRequest.predicate = NSPredicate(format: "(ANY contextLabels.labelID = %@) AND (%K == %@)",
                                                     labelID, Conversation.Attributes.userID, self.userID)
            }
            //TODO: - v4 fix sorting order
            fetchRequest.sortDescriptors = [NSSortDescriptor(key: #keyPath(Conversation.time), ascending: false)]
            fetchRequest.fetchBatchSize = 30
            fetchRequest.includesPropertyValues = true
            return NSFetchedResultsController(fetchRequest: fetchRequest, managedObjectContext: moc, sectionNameKeyPath: nil, cacheName: nil)
        }
    ...
    func cleanLocalMessageCache(_ completion: CompletionBlock?) {
        ...
        self.apiService.exec(route: getLatestEventID) { (task, response : EventLatestIDResponse) in
            ...
            self.cleanMessage().then { _ -> Promise<Void> in
                ...
                switch viewMode {
                case .singleMessage:
                    self.fetchMessages(byLable: Message.Location.inbox.rawValue, time: 0, forceClean: false, isUnread: false, completion: completionBlock)
                case .conversation:
                    ...
                    for conDict in conversationsDict {
                        ...
                        case IncrementalUpdateType.delete:
                            if let conversation = Conversation.conversationForConversationID(conversationEvent.ID, inManagedObjectContext: context) {
                                let labelObjs = conversation.mutableSetValue(forKey: Conversation.Attributes.contextLabel)
                                labelObjs.removeAllObjects()
        ...
        self.coreDataService.enqueue(context: self.coreDataService.operationContext) { (context) in
            for count in messageCounts {
                if let labelID = count["LabelID"] as? String {
                    ...
                    self.lastUpdatedStore.updateUnreadCount(by: labelID, userID: self.userID, count: unread, type: .message, shouldSave: false)
                }
            }
            ...
            let unreadCount: Int = self.lastUpdatedStore.unreadCount(by: Message.Location.inbox.rawValue, userID: self.userID, type: .message)
