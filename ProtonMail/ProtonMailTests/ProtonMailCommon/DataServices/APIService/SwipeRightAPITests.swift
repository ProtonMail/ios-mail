...
final class SwipeRightAPITests: XCTestCase {
    ...
    func testInit_withAllowedAction() throws {
        for action in allowedActions {
            let sut = try XCTUnwrap(SwipeRightRequest(action: action))
            XCTAssertEqual(sut.method, .put)
            XCTAssertEqual(sut.path, "/settings/mail/swiperight")
            let value = try XCTUnwrap(sut.parameters?["SwipeRight"] as? Int)
            switch action {
            case .trash:
    ...
    func testInit_withNotAllowedAction() {
        ...
        for action in actions {
            XCTAssertNil(SwipeRightRequest(action: action))
        }
    }
}
