...
class CacheServiceTest: XCTestCase {
    ...
    func testMoveMessageToArchive() {
        ...
        let unreadCountOfInbox: Int = lastUpdatedStore.unreadCount(by: label, userID: sut.userID, type: .message)
        let unreadCountOfArchive: Int = lastUpdatedStore.unreadCount(by: Message.Location.archive.rawValue, userID: sut.userID, type: .message)
        ...
        let newLabels: [String] = msg.getLableIDs()
        XCTAssertFalse(newLabels.contains(label))

        let unreadCountOfInboxAfterMove: Int = lastUpdatedStore.unreadCount(by: label, userID: sut.userID, type: .message)
        XCTAssertEqual(unreadCountOfInbox, unreadCountOfInboxAfterMove)

        let unreadCountOfArchiveAfterMove: Int = lastUpdatedStore.unreadCount(by: Message.Location.archive.rawValue, userID: sut.userID, type: .message)
    ...
    func testMoveUnreadMessageToArchive() {
        ...
        let newLabels: [String] = self.testMessage.getLableIDs()
        XCTAssertFalse(newLabels.contains(label))
    ...
    func testMoveMessageToTrash() {
        let label: String = Message.Location.inbox.rawValue
        let unreadCountOfInbox: Int = lastUpdatedStore.unreadCount(by: label, userID: sut.userID, type: .message)
        let unreadCountOfTrash: Int = lastUpdatedStore.unreadCount(by: Message.Location.trash.rawValue, userID: sut.userID, type: .message)
        ...
        let unreadCountOfTrashAfterMove: Int = lastUpdatedStore.unreadCount(by: Message.Location.archive.rawValue, userID: sut.userID, type: .message)
    ...
    func testMoveUnreadMessageToTrash() {
        ...
        let unreadCountOfInboxAfterMove: Int = lastUpdatedStore.unreadCount(by: label, userID: sut.userID, type: .message)
        XCTAssertEqual(unreadCountOfInboxAfterMove, 0)
        let unreadCountOfTrashAfterMove: Int = lastUpdatedStore.unreadCount(by: Message.Location.trash.rawValue, userID: sut.userID, type: .message)
    ...
    func testDeleteMessage() {
        ...
        let unreadCountOfInboxAfterDelete: Int = lastUpdatedStore.unreadCount(by: Message.Location.inbox.rawValue, userID: sut.userID, type: .message)
    ...
    func testMarkReadMessageAsRead() {
        ...
        let unreadCountOfInbox: Int = lastUpdatedStore.unreadCount(by: Message.Location.inbox.rawValue, userID: sut.userID, type: .message)
    ...
    func testLabelAndUnLabelMessage() {
        ...
        let labels: [String] = self.testMessage.getLableIDs()
    ...
    func testDeleteMessageByLabelID() {
        let msgID = self.testMessage.messageID
        let labelToDelete: String? = self.testMessage.getLableIDs().first
...
extension CacheServiceTest {
    func loadTestDataOfUnreadCount(defaultUnreadCount: Int, labelID: String) {
        lastUpdatedStore.updateUnreadCount(by: labelID, userID: sut.userID, count: defaultUnreadCount, type: .message, shouldSave: true)
