...
extension CacheServiceTest {
    func testUpdateCounterSyncOnMessage() {
        let labelIDs: [String] = self.testMessage.getLableIDs()
        ...
        for label in labelIDs {
            let msgUnReadCount: Int = lastUpdatedStore.unreadCount(by: label, userID: sut.userID, type: .message)
    ...
    func testPlusUnreadOnOneLabel() {
        ...
        let msgCount: Int = lastUpdatedStore.unreadCount(by: "0", userID: sut.userID, type: .message)
    ...
    func testUpdateUnreadLastUpdateTime() {
        ...
        sut.updateLastUpdatedTime(labelID: "02", isUnread: true, startTime: startDate, endTime: endDate, msgCount: 10, msgType: .message)

        var dataToCheck = self.lastUpdatedStore.lastUpdate(by: "02", userID: sut.userID, context: testContext, type: .message)!
        ...
        sut.updateLastUpdatedTime(labelID: "02", isUnread: true, startTime: startDate, endTime: laterEndDate, msgCount: 20, msgType: .message)

        dataToCheck = self.lastUpdatedStore.lastUpdate(by: "02", userID: sut.userID, context: testContext, type: .message)!
        XCTAssertFalse(dataToCheck.isUnreadNew)
        XCTAssertEqual(dataToCheck.unreadStart, startDate)
        XCTAssertEqual(dataToCheck.unreadEnd, endDate)

        let earlierEndDate = endDate.addingTimeInterval(-10000)
        sut.updateLastUpdatedTime(labelID: "02", isUnread: true, startTime: startDate, endTime: earlierEndDate, msgCount: 20, msgType: .message)
    ...
    func testUpdateReadLastUpdateTime() {
        let startDate = Date(timeIntervalSince1970: 1613816451)
        let endDate = Date(timeIntervalSince1970: 1614162051)
        sut.updateLastUpdatedTime(labelID: "02", isUnread: false, startTime: startDate, endTime: endDate, msgCount: 10, msgType: .message)
        ...
        let laterEndDate = endDate.addingTimeInterval(10000)
        sut.updateLastUpdatedTime(labelID: "02", isUnread: false, startTime: startDate, endTime: laterEndDate, msgCount: 20, msgType: .message)
        ...
        let earlierEndDate = endDate.addingTimeInterval(-10000)
        sut.updateLastUpdatedTime(labelID: "02", isUnread: false, startTime: startDate, endTime: earlierEndDate, msgCount: 20, msgType: .message)
