...
"""
    ...
    private var trackingMessageMarkedAfterFailure: String {
"""
<html>
    <body>
        Hey, I like tracking.
        <img src="E621E1F8-C36C-495A-93FC-0C247A3E6E5F" width="1" height="1" alt="">
    </body>
</html>
"""
    }
...
"""
    ...
    func testMarksSrcAttributeForOptionalReloadIfProxyRequestFails() async throws {
        ...
        let output = try sut.process(body: trackingMessage)
        assertHTMLsAreEqual(output.processedBody, trackingMessageMarkedAfterFailure)
        assertTrackingURLHasBeenListedForReload(output.summary.failedRequests)
    }

    func testMarksSrcAttributeForOptionalReloadInCaseOfNonImageResponses() async throws {
        apiServiceMock.downloadStub.bodyIs { _, urlString, destinationDirectoryURL, _, _, _, _, _, _, completion in
            let url = URL(string: urlString)!
    ...
    func testCachesTheData() async throws {
        _ = try sut.process(body: nonTrackingMessage)
        _ = try sut.process(body: nonTrackingMessage)
    ...
    private func assertTrackingURLHasBeenListedForReload(_ failedRequests: [UUID: URL], file: StaticString = #file, line: UInt = #line) {
        let expectedFailedRequests: [UUID: URL] = [
            Environment.uuid(): URL(string: trackingImageURL)!
        ]
