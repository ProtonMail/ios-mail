...
final class SenderImageRequestTests: XCTestCase {
    ...
    func testInit_withAddressAndUID() {
        ...
        XCTAssertEqual(
            sut.path,
            "/core/v4/images/logo?Address=\(email)&Mode=\(value)&UID=\(uid)&Format=png"
        )
    }
    ...
    func testInit_withAllParameters() {
        ...
        XCTAssertEqual(
            sut.path,
            "/core/v4/images/logo?Address=\(email)&Mode=\(value)&Size=\(32)&BimiSelector=\(bimiSelector)&UID=\(uid)&Format=png"
        )
    }
}
