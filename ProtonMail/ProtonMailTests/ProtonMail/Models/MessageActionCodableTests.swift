...
@available(*, deprecated, message: "Codable conformance is now autogenerated - remove these tests")
class MessageActionCodableTests: XCTestCase {
    ...
    func testUpdateContactGroup() throws {
        ...
        let action: MessageAction = .updateContactGroup(objectID: "updateGroupobjectID", name: "a name", color: "a color", addedEmailIDs: addedEmailIDs, removedEmailIDs: removedEmailIDs)
        let encoded = try JSONEncoder().encode(action)
        let decoded = try JSONDecoder().decode(MessageAction.self, from: encoded)
        XCTAssertEqual(action, decoded)
    }
