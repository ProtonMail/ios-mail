...
final class BlockSender {
    ...
    func execute(parameters: Parameters) throws {
        ...
        try dependencies.incomingDefaultService.performLocalUpdate(emailAddress: emailAddress, newLocation: .blocked)

        let task = QueueManager.Task(
            messageID: "",
            action: .blockSender(emailAddress: emailAddress),
