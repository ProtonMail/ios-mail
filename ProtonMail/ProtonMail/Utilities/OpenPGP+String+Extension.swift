...
extension String {
    ...
    func verifyMessage(verifier: [Data], userKeys: [Data], keys: [Key], passphrase: String, time : Int64) throws -> ExplicitVerifyMessage? {
        ...
        for key in keys {
            do {
                ...
                if let verifiedMessage =  try Crypto().decryptVerify(encrypted: self,
                                                                     publicKey: verifier,
    ...
    func encrypt(withKey key: Key, userKeys: [Data], mailbox_pwd: String) throws -> String? {
        let addressKeyPassphrase = try Crypto.getAddressKeyPassphrase(userKeys: userKeys, passphrase: mailbox_pwd, key: key)
