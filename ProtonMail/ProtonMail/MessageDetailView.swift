...
//                do {
//                    bodyText = try self.message.decryptBodyIfNeeded() ?? NSLocalizedString("Unable to decrypt message.")
...
//            let css = try! String(contentsOfFile: path!, encoding: NSUTF8StringEncoding)
    ...
    private func createHeaderView() {
        ...
        self.emailTitle.text = self.message.subject
    ...
    private func createFooterView() {
        let v = NSBundle.mainBundle().loadNibNamed("MessageDetailBottomView", owner: 0, options: nil)[0] as? UIView
    ...
    private func configureEmailDetailDateLabel() {
        ...
        if let _ = self.message.time {
            let tm = "" //TODO//messageTime.formattedWith("'On' EE, MMM d, yyyy 'at' h:mm a") ?? "";
...
extension MessageDetailView: UITableViewDelegate {
    ...
    private func openLocalURL(localURL: NSURL, keyPackage:NSData, fileName:String, forCell cell: UITableViewCell) {
        ...
        if let data : NSData = NSData(contentsOfURL: localURL) {
            ...
            let decryptData = try! data.decryptAttachment(keyPackage, passphrase: sharedUserDataService.mailboxPassword!)
