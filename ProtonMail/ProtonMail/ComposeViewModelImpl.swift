            ...
            if let atts = self.getAttachments() {
                for att in atts {
                    do {
                        ...
                        guard let newKeyPack = try session.getPublicSessionKeyPackage(str: key.public_key)?.base64EncodedString(options: NSData.Base64EncodingOptions(rawValue: 0)) else {
                            continue
                        }
