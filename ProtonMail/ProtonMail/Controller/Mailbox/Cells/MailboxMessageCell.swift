...
class MailboxMessageCell: MCSwipeTableViewCell, AccessibleCell {
    ...
    */
    ...
    func configureCell(_ conversation: Conversation, labelID: String) {
        ...
        let tempLabels = conversation.contextLabels.filtered(using: predicate) //TODO:: later need add lables exsiting check
        ...
        if conversation.contains(of: Message.Location.sent.rawValue) {
            labelsView.configLables( conversation.recipients, labels: [])
        } else if conversation.draft {
            labelsView.configLables( conversation.recipients, labels: [])
        } else {
            labelsView.configLables( conversation.senders, labels: [])
        }
        ...
        if let t : Date = conversation.time, let displayString = NSDate.stringForDisplay(from: t) {
