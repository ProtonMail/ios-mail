...
public class EncryptedSearchService {
    ...
    var esBGProcessingTaskRegistered: Bool = false
    var esBGAppRefreshTaskRegistered: Bool = false
...
extension EncryptedSearchService {
    ...
    func resizeSearchIndex(userID: String) {
        ...
        let sizeOfSearchIndex: Int64 = EncryptedSearchIndexService.shared.getSizeOfSearchIndex(for: userID).asInt64 ?? 0
        if userCachedStatus.storageLimit == -1 {
            ...
            if expectedESStates.contains(EncryptedSearchService.shared.getESState(userID: userID)) {
                DispatchQueue.global(qos: .default).async {
                    self.getTotalMessages(userID: userID) {
                        let numberOfMessageInSearchIndex: Int =
            ...
            if sizeOfSearchIndex > userCachedStatus.storageLimit {
                DispatchQueue.global(qos: .default).async {
                    if EncryptedSearchIndexService.shared
                        .shrinkSearchIndex(userID: userID,
                                           expectedSize: userCachedStatus.storageLimit) == false {
    ...
    private func cleanUpAfterIndexing(userID: String, metaDataIndex: Bool = false) {
        if metaDataIndex {
            ...
            if expectedESStates.contains(self.getESState(userID: userID)) {
                ...
                var indexingTime = CFAbsoluteTimeGetCurrent() - userCachedStatus.encryptedSearchIndexingStartTime
    ...
    func insertSingleMessageToSearchIndex(message: Message?, userID: String, completionHandler: @escaping () -> Void) {
        ...
        if EncryptedSearchIndexService.shared.checkIfSearchIndexExists(for: userID) {
            ...
            if let esMessage = esMessage {
                self.fetchMessageDetailForMessage(userID:
                                                    userID,
                                                  message:
                                                    esMessage) { [weak self] error,
                                                                             messageWithDetails in
                    if error == nil {
                        if let messageWithDetails = messageWithDetails {
                            self?.decryptAndExtractDataSingleMessage(message: MessageEntity(messageWithDetails.toMessage()),
                                                                     userID: userID,
                                                                     isUpdate: false) {
    ...
    func deleteMessageFromSearchIndex(message: MessageEntity,
                                      userID: String,
                                      completionHandler: @escaping () -> Void) {
    ...
    func convertMessageToESMessage(for message: Message) -> ESMessage {
        ...
        do {
            toList = try self.extractRecipientList(jsonString: message.toList)
            ccList = try self.extractRecipientList(jsonString: message.ccList)
            bccList = try self.extractRecipientList(jsonString: message.bccList)
        } catch {
        ...
        let time: Double = message.time?.timeIntervalSince1970 ?? 0
    ...
    private func extractRecipientList(jsonString: String) throws -> [ESSender?] {
        let decoder = JSONDecoder()
        var recipients: [ESSender?] = []
        if let jsonData: Data = jsonString.data(using: .utf8) {
            recipients = try decoder.decode([ESSender].self, from: jsonData)
        }
        return recipients
    }
    ...
    private func fetchSingleMessageFromServer(byMessageID messageID: String,
        ...
        self.apiService?.GET(request) { [weak self] _, responseDict, error in
            if error != nil {
                ...
                self?.messageService?
                    .cacheService
                    .parseMessagesResponse(labelID: LabelID(rawValue: Message.Location.allmail.rawValue),
                                           isUnread: false,
                                           response: response,
                                           idsOfMessagesBeingSent: self?.messageService?
                    ...
                    if urlResponse?.statusCode == 429 {
                        ...
                        if let retryTime = timeOut {
                            DispatchQueue.main.asyncAfter(deadline: .now() +
                                                          (Double(retryTime) ?? 0)) {
                                print("Error 429: Retry fetch after \(retryTime) seconds for message: \(message.id)")
    ...
    /*
    private func updateSearchIndexWithContentOfMessage(userID: String,
                                                       completionHandler: @escaping () -> Void) {
        let usersManager: UsersManager = sharedServices.get(by: UsersManager.self)
    ...
    /*private func downloadAndProcessPagesInParallel(userID: String, completionHandler: @escaping () -> Void) {
        self.pageSize = 150
        let numberOfPages: Int = Int(ceil(Double(userCachedStatus.encryptedSearchTotalMessages) /
        ...
        DispatchQueue.global(qos: .default).async {
            ...
            if let operation = processPageOperation {
                if let downloadPageQueue = self.downloadPageQueue {
                    ...
                    downloadPageQueue.maxConcurrentOperationCount =
                    self.downloadPageQueue?.maxConcurrentOperationCount ?? 1
                    downloadPageQueue.addOperation(operation)
    ...
    private func getMessage(messageID: String,
        ...
        self.messageService?.fetchedMessageControllerForID(MessageID(rawValue: messageID))
    ...
    func addMessageToSearchIndex(userID: String,
        ...
        if self.checkIfStorageLimitIsExceeded(userID: userID) == true {
            ...
            if let sizeOfIndex = sizeOfIndex {
                DispatchQueue.global(qos: .default).async {
                    _ = EncryptedSearchIndexService.shared
                        .shrinkSearchIndex(userID: userID,
        ...
        let rowID =
        EncryptedSearchIndexService.shared.addNewEntryToSearchIndex(userID: userID,
    ...
    func createESMessageFromSearchIndexEntry(userID: String,
        ...
        let encryptedMessageContent: EncryptedsearchEncryptedMessageContent? =
        EncryptedsearchEncryptedMessageContent(encryptionIV, ciphertextBase64: encryptedContent)
    ...
    func esRecipientToESSender(recipient: EncryptedsearchRecipient?) -> ESSender {
        ...
        return ESSender(name: "", address: "", group: "")
    }
    ...
    func refreshCache(userID: String) {
        ...
        let dbParams: EncryptedsearchDBParams? = EncryptedSearchIndexService.shared.getDBParams(userID)
        if let dbParams = dbParams {
            if let cipher = self.getCipher(userID: userID) {
    ...
    private func extractSearchResults(userID: String,
        ...
        if searchResults.length() == 0 {
            ...
            for index in 0...(searchResults.length() - 1) {
                group.enter()
                if let result: EncryptedsearchSearchResult = searchResults.get(index) {
                            ...
                            if self.isInternetConnection() {
                                ...
                                self.fetchSingleMessageFromServer(byMessageID: id) { [weak self] error in
                                    if error != nil {
                                        ...
                                        if let messageFromSearchIndex: Message =
                                        self?.createMessageFromPreview(userID: userID,
                                                                       searchResult: result) {
                                ...
                                self.createMessageFromPreview(userID: userID,
                                    ...
                                    print("Error: cannot create message from preview")
                                    group.leave()
                                }
                            }
                        }
    ...
    private func doIndexSearch(searcher: EncryptedsearchSimpleSearcher?,
        ...
        if let index: EncryptedsearchIndex = self.getIndex(userID: userID) {
            ...
            while searchState.isComplete == false && resultsFound < self.searchResultPageSize {
                ...
                do {
                    newResults = try index.searchNewBatch(fromDB: searcher,
                                                          cipher: cipher,
                                                          state: self.searchState,
                                                          batchSize: batchSize)
                    resultsFound += newResults?.length() ?? 0
                } catch {
    ...
    private func getIndex(userID: String) -> EncryptedsearchIndex? {
        let dbParams: EncryptedsearchDBParams? = EncryptedSearchIndexService.shared.getDBParams(userID)
        if let dbParams = dbParams {
            return EncryptedsearchIndex(dbParams)
        }
        return nil
    }
    ...
    private func doCachedSearch(searcher: EncryptedsearchSimpleSearcher?,
        ...
        var batchCount: Int = 0
        while searchState.cachedSearchDone == false && found < self.searchResultPageSize {
    ...
    func registerBGProcessingTask() {
        self.esBGProcessingTaskRegistered =
        BGTaskScheduler.shared.register(forTaskWithIdentifier: "ch.protonmail.protonmail.encryptedsearch_indexbuilding",
                                        using: nil) { bgTask in
            // swiftlint:disable force_cast
            self.bgProcessingTask(task: bgTask as! BGProcessingTask)
        }
        if !self.esBGProcessingTaskRegistered {
    ...
    private func cancelBGProcessingTask() {
        guard self.esBGProcessingTaskRegistered else {
            return
        }

        BGTaskScheduler.shared.cancel(taskRequestWithIdentifier:
                                        "ch.protonmail.protonmail.encryptedsearch_indexbuilding")
    }
    ...
    func registerBGAppRefreshTask() {
        self.esBGAppRefreshTaskRegistered =
        BGTaskScheduler.shared.register(forTaskWithIdentifier: "ch.protonmail.protonmail.encryptedsearch_apprefresh",
                                        using: nil) { bgTask in
            // swiftlint:disable force_cast
            self.appRefreshTask(task: bgTask as! BGAppRefreshTask)
        }
        if !self.esBGAppRefreshTaskRegistered {
    ...
    private func cancelBGAppRefreshTask() {
        guard self.esBGAppRefreshTaskRegistered else {
            return
        }
    ...
    private func sendIndexingMetrics(indexTime: Double, userID: String) {
        ...
        let indexingMetricsData = ["numMessagesIndexed": numMessagesIndexed,
                                    "indexSize": indexSize,
                                    ...
                                    "isRefreshed": userCachedStatus.encryptedSearchIsExternalRefreshed] as [String: Any]
    ...
    private func sendMetrics(metric: Metrics,
                             data: [String: Any],
        ...
        if metric == .search {
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + Double(delay)) {
                self.apiService?.metrics(log: "encrypted_search",
                                         title: title,
                                         data: data,
    ...
    private func createMessageFromPreview(userID: String,
                                          searchResult: EncryptedsearchSearchResult) -> Message? {
        if let msg: EncryptedsearchMessage = searchResult.message {
            ...
            let esMessage: ESMessage = ESMessage(id: msg.id_,
                                                 ...
                                                 unread: msg.decryptedContent?.unread ?? false ? 1 : 0,
                                                 type: 0,
                                                 ...
                                                 size: (searchResult.getBodyPreview()).utf8.count,
                                                 isEncrypted: 1,
                                                 expirationTime: Date(timeIntervalSince1970: Double(msg.decryptedContent?.expirationTime ?? 0)),
                                                 isReplied: msg.decryptedContent?.isReplied ?? false ? 1 : 0,
                                                 isRepliedAll: msg.decryptedContent?.isRepliedAll ?? false ? 1 : 0,
                                                 isForwarded: msg.decryptedContent?.isForwarded ?? false ? 1 : 0,
                                                 spamScore: nil,
    ...
    public func estimateIndexingTime() -> (estimatedTime: String?, time: Double, currentProgress: Int) {
        ...
        if userCachedStatus.encryptedSearchTotalMessages != 0 &&
            ...
            let remainingMessages: Double = Double(userCachedStatus.encryptedSearchTotalMessages -
                                                   userCachedStatus.encryptedSearchProcessedMessages)
            let timeDifference: Double = currentTime - userCachedStatus.encryptedSearchIndexingStartTime
            let processedMessageDifference: Double =
            Double(userCachedStatus.encryptedSearchProcessedMessages -
                   userCachedStatus.encryptedSearchPreviousProcessedMessages)

            // Estimate time (in seconds)
            estimatedTime = ceil((timeDifference / processedMessageDifference) * remainingMessages)
            // Estimate progress (in percent)
            currentProgress = Int(ceil((Double(userCachedStatus.encryptedSearchProcessedMessages) /
                                        Double(userCachedStatus.encryptedSearchTotalMessages)) * 100))
        }
    ...
    private func registerForNetworkChangeNotifications() {
        ...
        if self.networkMonitoringQueue == nil {
            ...
            self.networkMonitoringQueue = networkQueue
            self.networkMonitor?.start(queue: networkQueue)
    ...
    public func getSizeOfCachedData() -> (asInt64: Int64?, asString: String) {
        ...
        return (sizeOfCachedData, ByteCountFormatter.string(fromByteCount: sizeOfCachedData,
    ...
    public func calculateSizeOfAttachments() -> (asInt64: Int64?, asString: String) {
        ...
        return (sizeOfAttachments, ByteCountFormatter.string(fromByteCount: sizeOfAttachments,
                                                             countStyle: ByteCountFormatter.CountStyle.file))
    }
    ...
    func updateProgressedMessagesUI() {
        ...
        if userCachedStatus.encryptedSearchTotalMessages == 0 {
            ...
            self.viewModel?.currentProgress.value =
            Int(ceil((Double(userCachedStatus.encryptedSearchProcessedMessages) /
                      Double(userCachedStatus.encryptedSearchTotalMessages)) * 100))
        }
    }
            ...
            for node in body.getChildNodes() {
                ...
                if htmlString.contains(check: "mark") {
                    ...
                    for substring in stringArray {
                        ...
                        newHtmlString += substring.replacingOccurrences(of: "^\\s+",
                                                                        with: "",
    ...
    private func applyMarkUp(textNode: TextNode, keywords: [String]) throws -> Element? {
        ...
        if !positions.isEmpty {
            ...
            for position in positions {
                ...
                markNode = try markNode.appendChild(TextNode(self.substring(value: text,
                                                                            from: position.0,
                                                                            to: position.1), ""))
            ...
            if lastIndex < text.count {
                span = try span.appendChild(TextNode(self.substring(value: text,
                                                                    from: lastIndex,
                                                                    to: text.count), ""))
            }
            return span
    ...
    private func sanitizePositions(occurrences: [(Int, Int)]) -> [(Int, Int)] {
        ...
        if var previousValue: (Int, Int) = sorted.first {
    ...
    func addKeywordHighlightingToAttributedString(stringToHighlight: NSMutableAttributedString) -> NSMutableAttributedString {
        ...
        if !positions.isEmpty {
            for position in positions {
                ...
                let highlightColor = UIColor.dynamic(light: UIColor(hexString: "8A6EFF", alpha: 0.3),
                                                     dark: UIColor(hexString: "8A6EFF", alpha: 0.3))
                let highlightedAttributes = [NSAttributedString.Key.backgroundColor: highlightColor]
    ...
    func getTotalAvailableMemory() -> Double {
        ...
        let totalMb = Float(ProcessInfo.processInfo.physicalMemory)
    ...
    private func getCurrentlyAvailableAppMemory() -> Double {
        ...
        let usedMb = Float(taskInfo.phys_footprint)
        let totalMb = Float(ProcessInfo.processInfo.physicalMemory)
